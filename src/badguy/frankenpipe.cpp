...
static const Vector SUCK_TARGET_OFFSET = Vector(-16,-32);
static const float SUCK_TARGET_SPREAD = 8;

Frankenpipe::Frankenpipe(const ReaderMapping& mapping) :
  Boss(mapping, "images/creatures/frankenpipe/frankenpipe.sprite", LAYER_OBJECTS - 10),
  mystate(STATE_IDLE),
  willowisp_timer(),
  willo_spawn_y(0),
  willo_radius(200),
  willo_speed(1.8f),
  willo_color(0),
  glow_sprite(SpriteManager::current()->create("images/creatures/frankenpipe/frankenpipe-glow.sprite")),
  colorchange_timer(),
  suck_timer(),
  root_timer(),
  treecolor(0),
  ...
  willowisps()
{
  mapping.get("hud-icon", m_hud_icon, "images/creatures/frankenpipe/hudlife.png");
...
void
Frankenpipe::active_update(float dt_sec)
{
...
bool
Frankenpipe::is_color_deadly(Color color) const
{
...
void
Frankenpipe::willowisp_died(TreeWillOWisp* willowisp)
{
...
void
Frankenpipe::draw(DrawingContext& context)
{
  Boss::draw(context);
...
HitResponse
Frankenpipe::collision(MovingObject& other, const CollisionHit& )
{
...
void
Frankenpipe::spawn_lantern()
{
  Sector::get().add<Lantern>(m_col.m_bbox.get_middle() + SUCK_TARGET_OFFSET);
}

std::vector<Direction>
Frankenpipe::get_allowed_directions() const
{
  return {};
}

void
Frankenpipe::on_flip(float height)
{
