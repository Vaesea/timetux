//  SuperTux - Boss "Mummy", aka modified "Yeti"
...
namespace
{
const float RUN_VX = 450; /**< Horizontal speed while running. */
...
Mummy::Mummy(const ReaderMapping& reader) :
  Boss(reader, "images/creatures/mummy/mummy.sprite"),
  m_state(),
  m_state_timer(),
  m_safe_timer(),
  m_stomp_count(),
  ...
  m_jumped()
{
  reader.get("hud-icon", m_hud_icon, "images/creatures/mummy/hudlife.png");
  m_hud_head = Surface::from_file(m_hud_icon);
  SoundManager::current()->preload("sounds/mummy_gna.wav");
  SoundManager::current()->preload("sounds/mummy_roar.wav");
...
void
Mummy::draw(DrawingContext& context)
{
...
void
Mummy::active_update(float dt_sec)
{
  ...
  switch (m_state) {
    ...
    case BE_ANGRY:
      if (m_state_timer.check() && on_ground())
      {
        ...
        SoundManager::current()->play("sounds/mummy_gna.wav", get_pos());
      }
      break;
    case SQUISHED:
      {
...
bool
Mummy::collision_squished(MovingObject& object)
{
...
HitResponse
Mummy::collision_badguy(BadGuy& badguy, const CollisionHit& hit)
{
  badguy.kill_fall();
  return FORCE_MOVE;
}
...
void
Mummy::kill_squished(GameObject& object)
{
...
void Mummy::take_hit(Player& )
{
  ...
  SoundManager::current()->play("sounds/mummy_roar.wav", get_pos());
  m_lives--;
...
void
Mummy::collision_solid(const CollisionHit& hit)
{
...
Mummy::SnowExplosionParticle::SnowExplosionParticle(const Vector& pos, const Vector& velocity)
...
Mummy::get_allowed_directions() const
{
  return {};
}
